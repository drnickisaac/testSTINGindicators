...
---
  ...
  In this document, we're exploring the properties of the model proposed for Minimum Viable Scheme for EU Pollinator Monitoring Scheme. In this iteration, there are two data types: pan traps and transect walks. Both data types are surveyed on the same day each year. There are `v` visits to `n` sites: both numbers can be modified. We first define the model, then simulate data using user-defined parameters. We then try to recover these parameters for one simulated dataset. At present, the only parameters shared between the data types are those the the detection phenology.
...
mcSim <- nimbleModel(modelcode,
                     constants = list(nsite = nSites, 
                                   nvisit = nVisits,
                                   #Comment: NIMBLE has issues extracting the list, using the dollar  sign made the magic.
...
model <- nimbleModel(code = modelcode, 
                     ...
                     inits = list(z = apply(CmcSim$y1 > 0, 1, max), 
                                 ...
                                 # Comment: mu.psi and mu.lambda are nor in the model.
                                 # Comment: What did you want to define here?
                                 #changing those to alpha.s
                                  alpha.s = simSum["naiveOcc"], 
                                  #mu.lambda = simSum["meanCount_1"],
...
occMCMC <- buildMCMC(model, 
                     monitors = c(#'mu.psi', "mu.lambda", Comment: Have taken this out of the model because it is not defined in the model
                     'alpha.s',
                                  'alpha.p', "beta1", "beta2"), 
                     thin = 3, 
