...
modelcode <- nimbleCode({
  ...
  for (i in 1:nsite) {
    for (j in 1:nvisit) {
      ...
      Py[i,j] <- z[i] * p1[i,j]
      p1[i,j] <- max.p1 * phScale * f_JD[JulDate[i,j]]

       ##### transects 
       y2[i,j] ~ dpois(lambdaThin[i,j]) # Observed counts
       lambdaThin[i,j] <- lambda[i] * phScale * f_JD[JulDate[i,j]]
  }}
...
occMCMC <- buildMCMC(model, 
                     monitors = c('psi', "mu.lambda", 'max.p1', "beta1", "beta2"),# "f_JD"), 
                     thin = 3, 
...
system.time(
runMCMC_samples <- runMCMC(CoccMCMC, nburnin = 17500, niter = 20000, nchains = 3, samplesAsCodaMCMC = T)
