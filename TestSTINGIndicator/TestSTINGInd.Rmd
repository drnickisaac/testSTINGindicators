...
sim.pars <- c(mu.psi = 0.5, # proportion of occupied sites 
              mu.lambda = 30, # mean abundance across sites (at peak of season)
              alpha.p = -2, # logit probability of detection 
...
modelcode <- nimbleCode({
  ...
  for (i in 1:nsite) {
    ...
    log(lambda[i]) <- cloglcog(psi[i])
    logit(psi[i]) <- logit(mu.psi) + eta[i] 
  ...
  ######################### priors
  tau.si <- 1/(sigma.psi * sigma.psi) 
  sigma.si ~ T(dt(0, 1, 1), 0, Inf) # Half Cauchy
  mu.lambda ~ T(dt(0, 1, 1), 0, Inf) # Half Cauchy
  tau.psi <- T(dt(0, 1, 1), 0, Inf) # Half Cauchy
  ...
  phScale <- 1/max(f_JD[1:365]) # scaling parameter to ensure that phScale * f_JD = 1 at the peak of the phenology, i.e. where JulDate = beta1
...
mcSim <- nimbleModel(modelcode,
                     constants = list(nsite = nSites, 
                                   ...
                                   mu.psi = sim.pars["mu.psi"],
                                   mu.lambda = sim.pars["mu.lambda"],
