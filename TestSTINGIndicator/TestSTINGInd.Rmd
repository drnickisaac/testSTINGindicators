...
sim.pars <- c(psi = 0.5, # proportion of occupied sites 
              ...
              beta1 = 200, # date when detectability is highest (zero = 1st July)
#              beta1 = 10, # date when detectability is highest (zero = 1st July)
              beta2 = 20) # standard deviation of detection phenology
maxDate <- 250#120 # latest survey date. 120 corresponds to 120 days after 1 July, i.e. 28 October
minDate <- 100#-80 # earliest survey date. -80 corresponds to 80 days before 1 July, i.e. 11 April
```
...
modelcode <- nimbleCode({
  ...
  for (i in 1:nsite) {
    for (j in 1:nvisit) {
      ...
      Py[i,j] <- z[i] * p1[i,j]
      p1[i,j] <- max.p1 * beta3 * f_JD[cJulDate[i,j]]
#      p1[i,j] <- max.p1 * beta3 * f_JD[182 + cJulDate[i,j]]
# NB this assumes detection probability at 1 January is zero. Resolve with a nugget effect?      
  }}
...
#      lambdaThin[i,j] <- lambda[i] * beta3 * f_JD[182 + cJulDate[i,j]]
      lambdaThin[i,j] <- lambda[i] * beta3 * f_JD[cJulDate[i,j]]
  }}
  ...
  beta1 ~ dunif(1, 365) # peak detectability/activity: zero = 1 July
...
mcSim <- nimbleModel(modelcode,
                     ...
                     constants = list(nsite = nSites, 
                                   nvisit = nVisits,
                                   beta1 = sim.pars["beta1"], beta2 = sim.pars["beta1"],
                                   psi = sim.pars["psi"],
...
model <- nimbleModel(code = modelcode, 
                     constants = list(nsite = nSites, 
                                      nvisit = nVisits,
                                      nT = array(data = 5, dim = c(nSites, nVisits)),
                                      cJulDate = round(array(runif(n = nSites*nVisits, min = minDate, max = maxDate), 
...
```{r diagnostics}
# Rhat
gelman.diag(runMCMC_samples)  # this used to work - possibly psi.fs is causing problems
